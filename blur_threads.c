...
void *gaussianWrapper(void *arguments) {
    struct BlurArgs *args = (struct BlurArgs*)arguments;
    gaussian_blur(args -> src, args -> dst, args -> width, args -> height, args -> sigma, args -> ksize);
}
...
int main(int argc, char ** argv)
{
    ...
    pthread_t r, g, b;
    struct BlurArgs argr, argg, argb;
    ...
    argb.src = dataB;
    argb.dst = dstB;
    argr.width = argg.width = argb.width = width;
    argr.height = argg.height = argb.height = height;
    argr.sigma = argg.sigma = argb.sigma = sigma;
    argr.ksize = argg.ksize = argb.ksize = blur_size;

    pthread_create(&r, NULL, &gaussianWrapper, (void*)&argr);
    pthread_create(&g, NULL, &gaussianWrapper, (void*)&argg);
    pthread_create(&b, NULL, &gaussianWrapper, (void*)&argb);
